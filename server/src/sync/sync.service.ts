...
export class SyncService {
  ...
  pull(userId: string, lastSync: Date | null) {
    const since = lastSync ?? new Date(0);

    const profiles = [...this.store.profiles.values()].filter(
      (p) => p.updatedAt > since,
    );
    const likes = [...this.store.likes.values()].filter(
      (l) => l.updatedAt > since && (l.fromUserId === userId || l.toUserId === userId),
    );
    const matches = [...this.store.matches.values()].filter(
      (m) => m.updatedAt > since && (m.userId1 === userId || m.userId2 === userId),
    );
    const userMatchIds = new Set(
      [...this.store.matches.values()]
        .filter((m) => m.userId1 === userId || m.userId2 === userId)
        .map((m) => m.id),
    );
    const messages = [...this.store.messages.values()].filter(
      (msg) => msg.updatedAt > since && userMatchIds.has(msg.matchId),
    );
    const spots = [...this.store.sharedSpots.values()].filter(
      (s) => s.updatedAt > since && s.userId === userId,
    );
